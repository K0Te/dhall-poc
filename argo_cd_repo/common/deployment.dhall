...
let rollingDep =
      k8.RollingUpdateDeployment::{
      , maxSurge = Some ma
      , maxUnavailable = Some ma
      }

let strategy = k8.DeploymentStrategy::{ rollingUpdate = Some rollingDep }

let mkContainer =
      \(envVars : List k8.EnvVar.Type) ->
        k8.Container::{
        , name = "service_name"
...
let mkPodTemplateSpec =
      \(envVars : List k8.EnvVar.Type) ->
        k8.PodTemplateSpec::{ spec = Some (mkPodSpec envVars) }

let selector =
      k8.LabelSelector::{
      , matchLabels = Some
        [ { mapKey = "tier", mapValue = "backend" }
        , { mapKey = "name", mapValue = "service_name" }
        ]
      }
